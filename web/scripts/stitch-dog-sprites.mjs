...
const ONESHOT = new Set(['found'])
const SEAM_OK = MASK * MASK * 0.05
for (const [name, c] of clipList) {
  if (ONESHOT.has(name) || c.frames.length <= 8) continue
...
writeFileSync(resolve(root, '../app/composeApp/src/commonMain/kotlin/fyi/blep/ui/screens/DogFrames.kt'),
  ...
  `    /** frame count per clip (after loop-trim), indexed by DogClip.ordinal. */\n` +
...
clipList.forEach(([n], i) => {
  const m = clipMasks[i]; if (ONESHOT.has(n)) { console.log(`  ${n}: one-shot (no loop)`); return }
