...
while read -r folder lang country full cjk <&3; do
  ...
  IFS='|' read -r h1 s1 h2 s2 h3 s3 h4 s4 h5 s5 h6 s6 h7 s7 tag <<< "$(caps "$lang")"
  ...
  cap_portrait  "$raw/07.png" "$h7" "$s7" "$out/07.png"; cap_landscape "$raw/07.png" "$h7" "$s7" "$cr/07.png"
